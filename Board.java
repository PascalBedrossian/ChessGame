...
public class Board extends GridPane
{
    ...
    /**
     * Method that checks if the king can move on a specific
     * location. It basically check if its a legal move
     * 
    ...
    /**
     * Method that checks if the King can move after being checked
     * on any possible location on the board
     * 
    ...
    /**
     * Method that update the copied board for a king only,
     * and checks if by doing all possible moves for king
     * is still in check or not, or if its a legal move
     * 
    ...
    /**
     * Method that determines if any piece can defend
     * its own king by eating the opponent attacking
     * piece in a check situation
     * 
     * @param color a boolean for the color of the player trying to eat the opponent piece
    ...
    /**
     * Method that determines if any piece can block the check from opponent
     * by trying all the possible legal moves, and see if its still a check
     * for the king.
     * 
    ...
    /**
     * Method for castleDoneLegally White color that checks if
     * the castle is being blocked by an opponent piece
     * 
    ...
    /**
     * Method for castleDoneLegally Black color that checks if 
     * the castle is being blocked by an opponent piece
     * 
    ...
    /**
     * Method that checks if the castling can be done with the rook
     * and make sure that both pieces did not move already in the game
     * and that no opponent piece is blocking the castle
     * 
    ...
    private boolean castlingTry(Tile oldTile, Tile newTile){
        ...
        if(selectedPiece.color){ //White King    
            if(newTile.getX() == 6 && whiteRookR != null && !whiteRookR.hasMoved
                && tiles[5][0].getPiece() == null){ //right and no pieces between
            ...
            if(newTile.getX() == 2 && whiteRookL != null && !whiteRookL.hasMoved 
                && !castlingAvailable && tiles[1][0].getPiece() == null
                && tiles[3][0].getPiece() == null){//left and no pieces between
            ...
            if(newTile.getX() == 6 && blackRookR != null && !blackRookR.hasMoved
                && tiles[5][7].getPiece() == null){ //right and no piece between
                if (castleDoneLegally(selectedPiece.color, true)){//no checks onProcess
                    newTile.setPiece(oldTile.removePiece());
            ...
            if(newTile.getX() == 2 && blackRookL != null && !blackRookL.hasMoved 
                && !castlingAvailable && tiles[1][7].getPiece() == null
                && tiles[3][7].getPiece() == null){//left and no pieces between
                if (castleDoneLegally(selectedPiece.color, false)){//no checks onProcess
                    newTile.setPiece(oldTile.removePiece());
    ...
    /**
     * Method that displays the status of a king after a 
     * move have been done
     */
    ...
    /**
     * Check if everything applies then process a move after
     * it has been made by a player and set the updates of the board
     * 
     * @param p a MoveInformation
     * @param toDisplay a boolean that determines if display the Alert is on the current board
     * and that the method is being runned on a copied board or actual board.
     * @return a boolean
     */
    private boolean processMove(MoveInformation p, boolean toDisplay) {
        ...
        else{continueGame = true;}//turn was disabled so just continue game normally
    ...
    /**
     * Method that checks if processing a piece move do not occur
     * in check in player's own king, and process the move on
     * a copied board to check the alternative result.
     * 
     * @param p a MoveInformation
    ...
    private boolean pawnProcessingMove(MoveInformation p, Tile newTile, Tile oldTile, boolean toDisplay) {
        ...
        if (p.getNewY() == 7) { // White pawnPromotion
            ...
            else if(toDisplay && illegalDisplayEnabled){GUI.displayIllegalInfo();}
    ...
    private boolean IsPawnEnPassanLegal(MoveInformation m) {
        ...
        if (pawnEnPassant) {//If last move was of 2 tiles for a pawn
            ...
            if (tileToAttack.getPiece().color != pieceColor){//not the same color
                if(m.getNewX() == tileToAttack.getX() && (m.getOldX() == tileToAttack.getX() - 1 
                    || m.getOldX() == tileToAttack.getX() + 1)){ //both pawns are next to each others
                    if(pieceColor){//dealing with white pawn
                        if(m.getNewY() == tileToAttack.getY() + 1
                            && m.getOldY() == tileToAttack.getY()){//move goes positive in y
                                validMove = true;
                        }   
                    }
                    else if(m.getNewY() == tileToAttack.getY() - 1
                        && m.getOldY() == tileToAttack.getY()){//black pawn goes negative in y
                            validMove = true;
                    } 
                }
            }
    ...
    /**
     * Method that determines if a move made on the chess board is
     * a move within the available list of moves
     * 
     * @param p a MoveInformation 
     * @return a boolean
     */
    ...
    /**
     * Method that gets the ColorOrder to know how the board
